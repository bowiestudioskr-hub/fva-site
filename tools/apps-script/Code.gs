...
function collect(days) {
  ...
  try {
    ...
    if (days === 1 || yday) {
      /* ⚠ 오늘은 「지금 시각까지만」 그렸더니, 새벽 세 시에 보면 칸이 넷뿐이라
         막대 하나가 화면 4분의 1을 차지해 서로 겹쳐 보였다.
         하루는 늘 24칸이다. 아직 안 온 시간은 빈 칸으로 두면 모양도 뜻도 맞다. */
      const nowH = 23;
      /* ⚠ GA4 의 hour 는 '0','1','2' … 로 온다. 앞에 0 을 붙여 '00' 으로 찾으면
         0~9시가 영영 안 맞는다. 실제로 그동안 오전 0~9시 막대가 통째로 비어 있었고,
         새벽에 보면 그래프가 아예 안 그려졌다. 두 표기 다 받아 둔다. */
      for (let h = 0; h <= nowH; h++) {
        const k = String(h);
        const g = got[k] || got[(h < 10 ? '0' : '') + h] || { users: 0, views: 0 };
...
function pretty(v) {
  const m = {
    ...
    /* ⚠ 「아직 집계 중」이라고 적었다가 틀렸다. 하루 지나도 안 붙는다.
       2026-08-19 에 89명 중 57명이 이랬는데, 그 대부분이 **내가 사이트를 점검하려고
       띄운 헤드리스 크롬**이었다. 그날부터 태그가 헤드리스를 걸러낸다.
       그래도 남는 것은 진짜로 출처를 모르는 방문이다 — 그대로 적는다. */
    '(not set)': '출처 안 잡힘',
    '(data not available)': '출처 안 잡힘',
